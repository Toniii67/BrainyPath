...
import { courseApi, useAddAnswerInQuestionMutation, useAddNewQuestionMutation, useAddReplyInReviewMutation, useAddReviewInCourseMutation, useGetCourseDetailsQuery } from '@/redux/features/courses/coursesApi';
...
const CourseContentMedia = ({ data, id, activeVideo, setActiveVideo, user, refetch }: Props) => {
    ...
    // const [rating, setRating] = useState(isReviewExist ? isReviewExist.rating : 0);
    ...
    }, [isSuccess, error, refetch, answerSuccess, answerError, reviewSuccess, reviewError, courseRefetch, replySuccess, replyError])
                    ...
                    <div>
                        {
                            data[activeVideo]?.links.map((item: any, index: number) => (
                                ...
                                <div className='mb-5'>
                                    ...
                                    <a className='inline-block text-[#4395c4] 800px:text-[20px] 800px:pl-2' href={item.url}>{item.url}</a>
                                </div>
                            ))
                        }
                    </div>
                ...
                activeBar === 2 && (
                    <>
                        <div className='flex w-full'>
                            <Image
                                src={user.avatar ? user.avatar.url : "https://res.cloudinary.com/dclrjp3el/image/upload/v1716393717/avatars/phfjrqln8mpwpjwg5lor.png"}
                                width={20}
                                height={10}
                                alt=""
                                className='w-[45px] h-[43px] rounded-full object-cover'
                            />
                            <textarea
                                name=''
                                value={question}
                                ...
                                {(course?.reviews && [...course.reviews].reverse()).map((item: any, index: number) => (
                                    ...
                                    <div className='w-full my-5 dark:text-white text-black'>
                                        <div className='w-full flex'>
                                            <div>
                                                <Image
                                                    src={item.user.avatar ? item.user.avatar.url : "https://res.cloudinary.com/dclrjp3el/image/upload/v1716393717/avatars/phfjrqln8mpwpjwg5lor.png"}
                                                    width={20}
                                                    height={10}
                                                    alt=""
                                                    className='w-[45px] h-[43px] rounded-full object-cover'
                                            ...
                                            item.commentReplies.map((i: any, index: number) => (
                                                ...
                                                <div className='w-full flex 800px:ml-16 my-5'>
                                                    <div className='w-[50px] h-[50px]'>
                                                        <Image
                                                            src={i.user.avatar ? i.user.avatar.url : "https://res.cloudinary.com/dclrjp3el/image/upload/v1716393717/avatars/phfjrqln8mpwpjwg5lor.png"}
                                                            width={20}
                                                            height={10}
                                                            alt=""
                                                            className='w-[45px] h-[43px] rounded-full object-cover'
                                                    ...
                                                    <div className="pl-2">
                                                        <div className='flex items-center'>
                                                            <h5 className='text-[20px]'>{i.user.name}</h5> {i.user.role === 'admin' && <VscVerifiedFilled className='text-[#0095F6] ml-2 text-[20px] mt-[4px]' />}
                                                        </div>
                                                        <p>{i.comment}</p>
        ...
        <>
            <div className="my-4">
                ...
                <div className="w-full flex">
                    ...
                    <BiMessage size={20} className='cursor-pointer text-[#000000b8] dark:text-[#ffffff83] mt-[4px]' />
                    <span className='pl-1 mt-[-1px] cursor-pointer text-[#000000b8] dark:text-[#ffffff83]'>
                            ...
                            {item.questionReplies.map((item: any) => (
                                ...
                                <div className='w-full flex 800px:ml-16 my-5 text-black dark:text-white'>
                                    ...
                                    <div className="pl-3">
                                        ...
                                        <div className='flex items-center'>
                                            <h5 className='text-[20px]'>{item.user.name}</h5> {item.user.role === 'admin' && <VscVerifiedFilled className='text-[#0095F6] ml-2 text-[20px] mt-[4px]' />}
                                        </div>
                                        <p>{item.answer}</p>
                                        <small className='text-[#ffffff83]'>
                            ...
                            <>
                                <div className='w-full flex relative dark:text-white text-black'>
                                    <input
                                        ...
                                        className={`block 800px:ml-12 mt-2 outline-none bg-transparent border-b border-[#00000027] dark:border-[#fff] dark:text-white text-black p-[5px] w-[95%] ${answer === "" || answerCreationLoading && 'cursor-not-allowed'}`}
                                    />
                                    <button
                                        type='submit'
                                        className='absolute right-0 bottom-1'
